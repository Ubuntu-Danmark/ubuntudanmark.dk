...
function wp2bb_widget($args) {
    ...
    if ($forums)
        $query = $query . " AND p.forum_id IN (" . $forums . ")";
...
function wp2bb() {
    ...
    global $db, $config, $phpEx, $template, $phpbb_root_path;
    $id = get_the_ID();
...
function wp2bb_publish($id) {
    ...
    global $phpbb_root_path, $sql_db, $db, $username, $usercolour, $auth;
